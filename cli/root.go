// Package cli assembles the nws command tree from the nws
...
// NewApp assembles the kit application from the nws domain. The
...
//
// To add a command, declare it in nws/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := nws.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(nws.Domain{}).Register(app)

...
describes what users get. Pick a format with `-o`, or let nws choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
nws <command> -o table   # aligned columns for reading
nws <command> -o jsonl   # one JSON object per line, for piping
nws <command> -o json    # a single JSON array
nws <command> -o csv     # spreadsheet friendly
nws <command> -o tsv     # tab-separated
nws <command> -o url     # just the URL column
nws <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
nws <command> --fields id,title,url
```
...
```bash
nws <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
nws <command>            # a table, because this is a terminal
nws <command> | wc -l    # JSONL, because this is a pipe
```
